...
balance_list_t Account::get_balances() {
    auto res = api_request<balance_list_t, balance_t>(BALANCES_END_P, Type::ACCOUNT, "");
    return res;
}


balance_t Account::get_balance(const std::string &currency) {
    auto res = api_request<balance_t, balance_t>(BALANCE_END_P, Type::ACCOUNT, "currency=",
                                                 currency);
    return res;
...
deposit_address_t Account::get_deposit_address(const std::string &currency) {
    auto res = api_request<deposit_address_t, deposit_address_t>(DEPOSITADDR_END_P, Type::ACCOUNT,
                                                                 "currency=", currency);
    return res;
...
uuid_t Account::withdraw(const std::string &currency, const float &quantity,
    ...
    auto res = api_request<uuid_t, uuid_t>(WITHDRAW_END_P, Type::ACCOUNT,
...
order_t Account::get_order(const std::string &uuid) {
    auto res = api_request<order_t, order_t>(ORDER_END_P, Type::ACCOUNT, "uuid=", uuid);
    return res;
}
...
//               _api_call->dispatch("account/getorderhistory?", Type::ACCOUNT, "market=", market):
//               _api_call->dispatch("account/getorderhistory?", Type::ACCOUNT, "");
...
withdrawal_history_ent_list_t Account::get_withdrawal_history(const std::string &currency) {
    auto res = api_request<withdrawal_history_ent_list_t, withdrawal_history_ent_t>(
            WITHDRAWAL_HIS_END_P, Type::ACCOUNT, "currency=", currency);
    return res;
}
